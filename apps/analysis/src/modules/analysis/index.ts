...
import { getMeasurementsForPond, pondExists, getAllPondIds } from "./repository";
import {
 ...
 */
async function handleAnalysis(query: { pondId: string; startDate?: string; endDate?: string; window?: string }) {
  ...
  if (measurements.length === 0) {
    throw new Error(`No measurements found for pond '${pondId}' in the specified time range`);
  }
  
  // Check data coverage (at least 70% of expected parameters)
  const expectedParameters: ParameterCode[] = [
    "temperature", "ph", "salinity", "dissolved_oxygen", "turbidity"
  ];
  
  const hasSufficientCoverage = checkDataCoverage(measurements, expectedParameters);
  if (!hasSufficientCoverage) {
    console.warn(`Insufficient data coverage for pond ${pondId} in time range ${startDate.toISOString()} to ${endDate.toISOString()}`);
  ...
  const parameterTemporalScores = calculateParameterTemporalScores(normalizedByParameter);
  ...
  .get(
    ...
    },
    {
      query: t.Object({
        ...
        window: t.Optional(t.Union([t.Literal("7d"), t.Literal("30d"), t.Literal("90d"), t.Literal("custom")])),
      }),
    }
  )
  .get("/ponds", async () => {
